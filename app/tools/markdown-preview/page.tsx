    ...
    <ToolLayout title="Markdown Preview" description="Write Markdown and see the HTML preview in real-time.">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div>
          ...
          <textarea value={input} onChange={e => setInput(e.target.value)} className="w-full h-96 font-mono text-sm border border-gray-300 rounded-xl p-4 focus:ring-2 focus:ring-indigo-400 resize-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Preview</label>
          <div className="w-full h-96 border border-gray-200 rounded-xl p-4 bg-white overflow-auto prose prose-sm max-w-none" dangerouslySetInnerHTML={{ __html: md2html(input) }} />
        </div>
      </div>
    </ToolLayout>
  );
